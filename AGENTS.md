...
- **Bare-Metal Disaster Recovery (TUI Scripts):** NixitOS provides native interactive TUI scripts (`backup` and `restore`) located in `build_files/usr/bin/` to safely export and import the entire `/var/home` Btrfs subvolume to USB drives.
...
- **Engine Definition:** The GGUF engine compose and router config are maintained in this repo under `build_files/usr/share/nixit-gguf-engine/` and installed to `/usr/share/nixit-gguf-engine/`. The compose uses the upstream `ghcr.io/ggml-org/llama.cpp:server-intel` image directly — no local Containerfile needed. Runtime state only belongs under `/var` or user state directories; the engine must not depend on ad-hoc directories in the user's home.
