...
- Build OS image locally: `podman build -f Containerfile -t ariaos .`
...
- LLM context is capped at 32k for local inference; 128k profiles are explicitly excluded from config.ini.
...
- `build_files/usr/share/aria-gguf-engine/compose.yaml` -- Podman Compose definition for llama.cpp inference container with SYCL device passthrough and healthcheck.
